    ...
    public void onClientTick()
    {
        ...
        if(mc.player != null && !mc.options.hudHidden)
        {
            ...
            if(mc.currentScreen instanceof AbstractContainerScreen)
            {
                ...
                {
                	AbstractContainerScreen<?> container = (AbstractContainerScreen<?>) mc.currentScreen;
        ...
        RenderSystem.pushMatrix();
        {
            if(!RenderAvailableActionCallback.EVENT.invoker().onRenderAvailableActionEvent())
            {
                ...
                for(Integer button : actions.keySet())
                {
                    ...
                    int x = side == Action.Side.LEFT ? 5 : mc.getWindow().getScaledWidth() - 5 - size;
                    int y = mc.getWindow().getScaledHeight() + (side == Action.Side.LEFT ? leftIndex : rightIndex) * -15 - size - 5;
