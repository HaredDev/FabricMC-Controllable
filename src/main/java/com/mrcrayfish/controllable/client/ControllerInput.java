...
public class ControllerInput
{
    ...
    public void onClientTick()
    {
            ...
            {
                double mouseSpeed = Controllable.getOptions().getMouseSpeed() * mc.getWindow().getScaleFactor();
                targetMouseX += mouseSpeed * mouseSpeedX;
                targetMouseX = MathHelper.clamp(targetMouseX, 0, mc.getWindow().getWidth());
                targetMouseY += mouseSpeed * mouseSpeedY;
                targetMouseY = MathHelper.clamp(targetMouseY, 0, mc.getWindow().getHeight());
                lastUse = 100;
                moved = true;
            }
            ...
            if(Controllable.getController() != null && Controllable.getOptions().isVirtualMouse())
            {
                ...
                if(screen != null && (targetMouseX != prevTargetMouseX || targetMouseY != prevTargetMouseY))
                {
                    if(mc.overlay == null)
                    {
                        double mouseX = virtualMouseX * (double) mc.getWindow().getScaledWidth() / (double) mc.getWindow().getWidth();
                        double mouseY = virtualMouseY * (double) mc.getWindow().getScaledHeight() / (double) mc.getWindow().getHeight();
                        ...
                        {
                            double dragX = (targetMouseX - prevTargetMouseX) * (double) mc.getWindow().getScaledWidth() / (double) mc.getWindow().getWidth();
                            double dragY = (targetMouseY - prevTargetMouseY) * (double) mc.getWindow().getScaledHeight() / (double) mc.getWindow().getHeight();
    ...
    public void onScreenInit()
    {
        ...
        if(mc.currentScreen == null)
        {
            ...
            mouseSpeedX = 0.0;
            mouseSpeedY = 0.0;
            virtualMouseX = targetMouseX = prevTargetMouseX = (int) (mc.getWindow().getWidth() / 2F);
            virtualMouseY = targetMouseY = prevTargetMouseY = (int) (mc.getWindow().getHeight() / 2F);
        }
    }
        ...
        if(Controllable.getController() != null && Controllable.getOptions().isVirtualMouse() && lastUse > 0)
        {
            RenderSystem.pushMatrix();
            {
                ...
                if(minecraft.player == null || (minecraft.player.inventory.getCursorStack().isEmpty() || type == CursorType.CONSOLE))
                {
                    ...
                    RenderSystem.translated(mouseX / minecraft.getWindow().getScaleFactor(), mouseY / minecraft.getWindow().getScaleFactor(), 500);
    ...
    private void scrollCreativeTabs(CreativeInventoryScreen creative, int dir)
    {
        lastUse = 100;
            ...
            if(dir > 0)
            {
                if(creative.getSelectedTab() < ItemGroup.GROUPS.length - 1)
                {
                    helper.setSelectedTab(creative, ItemGroup.GROUPS[creative.getSelectedTab() + 1]);
                }
            }
            else if(dir < 0)
            {
                if(creative.getSelectedTab() > 0)
                {
                    helper.setSelectedTab(creative, ItemGroup.GROUPS[creative.getSelectedTab() - 1]);
                }
            }
    ...
    private void moveMouseToClosestSlot(boolean moving, Screen screen)
    {
        ...
        if(screen instanceof CreativeInventoryScreen)
        {
            ...
            int mouseX = (int) (targetMouseX * (double) mc.getWindow().getScaledWidth() / (double) mc.getWindow().getWidth());
            int mouseY = (int) (targetMouseY * (double) mc.getWindow().getScaledHeight() / (double) mc.getWindow().getHeight());
            ...
            for(Slot slot : guiContainer.getContainer().slotList)
            {
            ...
            if(closestSlot != null && (closestSlot.hasStack() || !mc.player.inventory.getCursorStack().isEmpty()))
            {
                ...
                int slotCenterX = (int) (slotCenterXScaled / ((double) mc.getWindow().getScaledWidth() / (double) mc.getWindow().getWidth()));
                int slotCenterY = (int) (slotCenterYScaled / ((double) mc.getWindow().getScaledHeight() / (double) mc.getWindow().getHeight()));
    ...
    private void handleCreativeScrolling(CreativeInventoryScreen creative, Controller controller)
    {
            ...
            creative.getContainer().scrollItems(currentScroll);
    }
    ...
    private void invokeMouseClick(Screen screen, int button)
    {
        ...
        if(screen != null)
        {
            ...
            mouseX = mouseX * (double) mc.getWindow().getScaledWidth() / (double) mc.getWindow().getWidth();
            mouseY = mouseY * (double) mc.getWindow().getScaledHeight() / (double) mc.getWindow().getHeight();
