...
public class Hooks
{
    ...
    public int[] drawScreen(Screen screen, int mouseX, int mouseY, float partialTicks)
    {
        ...
        if(Controllable.getController() != null && Controllable.getOptions().isVirtualMouse() && input.getLastUse() > 0)
        {
            ...
            mouseX = (int) (input.getVirtualMouseX() * (double) minecraft.window.getScaledWidth() / (double) minecraft.window.getWidth());
            mouseY = (int) (input.getVirtualMouseY() * (double) minecraft.window.getScaledHeight() / (double) minecraft.window.getHeight());
        }
